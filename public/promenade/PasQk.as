...
package {
	...
	var mheight:int = 600;//map領域の縦幅指定
	...
	var subbmdnewidth:Array = new Array();
	var subbmdneheight:Array = new Array();
	var n_floor:String;
	...
	var submapld2 : Loader = new Loader();
		...
		public function PasQk() {
			...
			function complete_meta(event:Event):void{
				...
				function complete_bmd(event:Event):void{//CCD読み込み終了後
					...
					b_len = bmd.maps.img.length();//ccd.xml内のContentの長さ（数）を格納
					bmd_mask = bmd.maps.masks.length();
					for(cn=0;cn<b_len;cn++){
						if(cn == 0){
							...
							subusebmd[cn-1]="image/"+bmd.maps[cn].img.@src;
							subbmdnewidth[cn-1] = bmd.maps[cn].img.@width;
							subbmdneheight[cn-1] = bmd.maps[cn].img.@height;
		...
		function start(){//ここからメイン					
			...
			function onComplete(event:Event):void{
				...
				var matrix2:Matrix = new Matrix();
					...
					for(i=0; i<b_len-1; i++){
						subpw = subbmdnewidth[i];
    					subph = subbmdneheight[i];
						pwe = ((subbmdnelng[i] - Number(bmdswlng))*(mwidth/clng)-(subbmdswlng[i] - Number(bmdswlng))*(mwidth/clng));
    					pns = ((subbmdnelat[i] - Number(bmdswlat))*(mheight/clat)-(subbmdswlat[i] - Number(bmdswlat))*(mheight/clat));
						...
						subbmp.x = (stage.stageWidth - 920 +((subbmdswlng[i] - Number(bmdswlng))*(mwidth/clng)));
						subbmp.y = (stage.stageHeight - 620 +mheight- ((subbmdnelat[i] - Number(bmdswlat))*(mheight/clat)));//*(600/500)); 
						addChild(subbmp);
					}
					
					for(i=0; i<p_len; i++){
						shape1.graphics.beginFill(0x00ff66);
						shape1.graphics.drawCircle(stage.stageWidth - 920 +((xml.Panoramas.Panorama.(@panoid == "pano"+i).coords.@lng - Number(bmdswlng))*(mwidth/clng)), stage.stageHeight - 620 +mheight- ((xml.Panoramas.Panorama.(@panoid == "pano"+i).coords.@lat - Number(bmdswlat))*(mheight/clat)),5);
