			...
			function complete_meta(event:Event):void{
				...
				function complete_bmd(event:Event):void{//CCD読み込み終了後
					...
					for(cn=0;cn<bmd_len;cn++){
						bmddata = bmd.maps.img.@src;//各contentの経度を格納
						usebmd="image/"+bmddata;
						bmdlat = bmd.maps.data.@lat;
						bmdlng = bmd.maps.data.@lng;
					}
		...
		function start(){//ここからメイン					
			...
			function comHandler(e:Event){
				...
				for(i=0;i<ccd_len1;i++){
					...
					if(cnt_name=="promenade"){cnt_name="散歩道"}
			...
			return function mapchange(event:Event):void {
				...
				//removeChild(sp);
						sp.x = (stage.stageWidth - 920 +((lng - Number(bmdlng))*1000000)*(600/705));
						sp.y = (stage.stageHeight - 620 +600- ((lat - Number(bmdlat))*1000000)*(600/709));//*(600/500)); 
					//trace(mapmarker.x);
					//addChild(sp);
			}
			}
